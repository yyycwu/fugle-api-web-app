import { FugleApi, TrialObject, QuoteObject, QuoteResult } from '../fugleApi/fugleApi.js'
import { FugleApiSocket } from '../fugleApi/fugleApiSocket.js';

const rowDivsMap = new WeakMap;
declare const quoteRowTemplate: HTMLTemplateElement;
declare global {
...
class SharedTimer {
	...
	static callbacks = new Map<SharedTimer, () => void>;
	...
	static stop(): boolean;
	static stop(sharedTimer: SharedTimer): boolean;
	static stop(sharedTimer?: SharedTimer) {
...
const enum CountTradeAt { Initial, Bid, Ask }
FugleApi.alert = console.error;
class QuoteGridGroup {
	metaDivs: HTMLCollectionOf<HTMLDivElement>;
	symbolIdDiv: HTMLDivElement;
	get symbolId() {
		return this.symbolIdDiv.textContent;
	}
	set symbolId(id: string) {
		this.symbolIdDiv.textContent = id;
	}
	symbolNameDiv: HTMLDivElement;
	get symbolName() {
		return this.symbolNameDiv.textContent;
	}
	set symbolName(name: string) {
		this.symbolNameDiv.textContent = name;
	}
	countTimesDiv: HTMLDivElement;
	get countTimes() {
		return +this.countTimesDiv.textContent;
	}
	set countTimes(count: number) {
		this.countTimesDiv.innerHTML = QuoteGridGroup.getColorHtml(
			count + '',
			this.getCountTradeColor()
		);
	}
	countVolumeDiv: HTMLDivElement;
	get countVolume() {
		return +this.countVolumeDiv.textContent;
	}
	set countVolume(volume: number) {
		this.countVolumeDiv.innerHTML = QuoteGridGroup.getColorHtml(
			volume + '',
			this.getCountTradeColor()
		);
	}
	countLastVolumeDiv: HTMLDivElement;
	get countLastVolume() {
		return +this.countLastVolumeDiv.textContent;
	}
	set countLastVolume(volume: number) {
		this.countLastVolumeDiv.innerHTML = QuoteGridGroup.getColorHtml(
			volume + '',
			this.getCountTradeColor()
		);
		if (volume < this.totalVolume / this.totalTransaction) {
			const span = this.countLastVolumeDiv.firstElementChild as HTMLSpanElement;
			span.style.fontWeight = 'normal';
		}
	}
	amplitudeDiv: HTMLDivElement;
	get amplitude() {
	...
	priceLowDiv: HTMLDivElement;
	get priceLow() {
		return +this.priceLowDiv.textContent || 0;
	}
	set priceLow(price: number | undefined) {
		this.priceLowDiv.innerHTML =
			typeof price === 'undefined' ?
				'-' :
				QuoteGridGroup.getColorHtml(
					QuoteGridGroup.toFixed(price),
					this.getPriceColor(price)
				);
	}
	bidPriceDivs: HTMLDivElement[];
	bidPrices = new Proxy<{ [key: number]: number | undefined }>(
		Object.create(null),
		{
			get: (_, key) => {
	...
	tradePriceDiv: HTMLDivElement;
	get tradePrice() {
		return +this.tradePriceDiv.textContent;
	}
	set tradePrice(price: number | undefined) {
		this.tradePriceDiv.innerHTML = typeof price === 'undefined' ? '' :
			QuoteGridGroup.getColorHtml(
				QuoteGridGroup.toFixed(price),
				this.getPriceColor(price, '#eee')
			);
	}
	askPriceDivs: HTMLDivElement[];
	askPrices = new Proxy<{ [key: number]: number | undefined }>(
		Object.create(null),
		{
			get: (_, key) => {
				const index = +key.toString();
	...
	priceHighDiv: HTMLDivElement;
	get priceHigh() {
		return +this.priceHighDiv.textContent || 0;
	}
	set priceHigh(price: number | undefined) {
	...
	changePercentDiv: HTMLDivElement;
	get changePercent() {
		return parseInt(this.changePercentDiv.textContent);
	}
	set changePercent(percent: number) {
		this.changePercentDiv.innerHTML = QuoteGridGroup.getColorHtml(
			Math.abs(percent).toFixed(1) + '%',
			percent > 0 ? 'sandybrown' : percent < 0 ? 'lightgreen' : 'inherit'
		);
	}
	priceLowLimitDiv: HTMLDivElement;
	get priceLowLimit() {
		return +this.priceLowLimitDiv.textContent;
	}
	set priceLowLimit(price: number | null) {
		this.priceLowLimitDiv.textContent = price === null ? '0' : QuoteGridGroup.toFixed(price);
	}
	bidVolumeDivs: HTMLDivElement[];
	bidVolumes = new Proxy<{ [key: number]: number | undefined }>(
		Object.create(null),
		{
			get: (_, key) => {
				const index = +key.toString();
	...
	tradeVolumeDiv: HTMLDivElement;
	get tradeVolume() {
		return +this.tradeVolumeDiv.textContent;
	}
	set tradeVolume(volume: number | undefined) {
		this.tradeVolumeDiv.textContent = typeof volume === 'undefined' ? '' : volume + '';
	}
	setTradeVolume(trade?: TrialObject) {
		if (
			this.lastTradeAt !== null &&
			Date.now() - Date.parse(this.lastTradeAt) > 5_000
		) { return; }
		this.tradeVolumeDiv.innerHTML = typeof trade === 'undefined' ? '' :
			QuoteGridGroup.getColorHtml(
				trade.volume + '',
				trade.price === trade.bid ? 'lawngreen'
					: trade.price === trade.ask ? 'sandybrown' :
						'#ddd'
			);
	}
	askVolumeDivs: HTMLDivElement[];
	askVolumes = new Proxy<{ [key: number]: number | undefined }>(
		Object.create(null),
		{
			get: (_, key) => {
				const index = +key.toString();
	...
	priceHighLimitDiv: HTMLDivElement;
	get priceHighLimit() {
		return +this.priceHighLimitDiv.textContent;
	}
	set priceHighLimit(price: number | null) {
		this.priceHighLimitDiv.textContent = price === null ? '∞' : QuoteGridGroup.toFixed(price);
	}
	priceReference: number;
	getPriceColor(price: number, flatColor = 'inherit', redColor = 'coral', greenColor = 'greenyellow') {
		return price > this.priceReference ? redColor :
			price < this.priceReference ? greenColor :
				flatColor;
	}
	setPriceLimitColorHtml(div: HTMLDivElement, price: number | undefined, volumeDivs: HTMLDivElement[], index: number) {
		if (typeof price === 'undefined') {
			div.textContent = '';
		} else {
			function getTransformHtml(text: string, color = 'inherit') {
				return '<span style="color: ' + color + ';position: absolute;transform: translate(-50%, -3px);">' + text + '</span>';
			}
			function setVolumeDivColor(color: string) {
			...
			if (
				price === this.priceHighLimit ||
				price === this.priceLowLimit
			) {
				div.innerHTML = getTransformHtml(
					price === this.priceHighLimit ? '漲停' : '跌停',
					this.getPriceColor(price, '#ddd', 'chocolate', 'yellowgreen')
				);
				setVolumeDivColor(price === this.priceHighLimit ? 'lightcoral' : 'lightgreen');
			} else if (price === this.tradePrice) {
				div.innerHTML = QuoteGridGroup.getColorHtml(
					text,
					this.getPriceColor(price, '#ddd', 'chocolate', 'yellowgreen')
				);
			} else {
	...
	dealsDivs: HTMLCollectionOf<HTMLDivElement>;
	lastDealPrice = 0 / 0;
	addDeal(
		price: number | undefined,
		volume: number,
		at = this.lastTradeAt ?? new Date().toISOString(),
		countTradeAt = this.countTradeAt
	) {
		if (typeof price === 'undefined') { return; }
		const div = this.dealsDivs[13],
			date = new Date(at);
		div.innerHTML =
			...
			'<span style="color: ' + (
				price > this.lastDealPrice ?
					...
					price < this.lastDealPrice ?
						'#bbf0bb' :
						'#eee'
			) + ';">' + QuoteGridGroup.toFixed(price) + '</b>\n' +
			'<span style="color: ' + (
				countTradeAt === CountTradeAt.Ask ?
					'lightcoral' :
					countTradeAt === CountTradeAt.Bid ?
						'lightgreen' :
						'#eee'
			) + ';">' + volume + '</b>';
	...
	bidsAsksLength: number;
	childrenDivs: HTMLDivElement[];
	tradeVolumeAtBid = 0;
	tradeVolumeAtAsk = 0;
	constructor(fragment: DocumentFragment) {
		const cloneFragment = quoteRowTemplate.content.cloneNode(true) as DocumentFragment;
		let metaDiv: HTMLDivElement,
			orderDiv: HTMLDivElement,
			dealsDiv: HTMLDivElement;
		this.childrenDivs = [
			this.symbolIdDiv,
			this.symbolNameDiv,
			this.countTimesDiv,
			...
			orderDiv,
			dealsDiv
		] = [...cloneFragment.children as HTMLCollectionOf<HTMLDivElement>];

		const orderDivs = orderDiv.children as HTMLCollectionOf<HTMLDivElement>;
		...
		const metaDivs = metaDiv.children as HTMLCollectionOf<HTMLDivElement>;
		...
		this.dealsDivs = dealsDiv.children as HTMLCollectionOf<HTMLDivElement>;

		const closeDiv = metaDiv.lastElementChild as HTMLDivElement;
		closeDiv.onclick = () => {
			this.remove();
		};
	...
	async initMeta() {
		...
		} } } = await FugleApi.meta(this.symbolId);
		...
		this.setMeta(
			canDayBuySell ? canDaySellBuy ?
				getBuyHtml() + getSellHtml() + '現沖' :
				'先' + getBuyHtml() + '現沖' :
				getColorHtml('禁現沖', 'gold'),
			market === 'TSE' ? '市' : market === 'OTC' ? '櫃' : market === 'ESB' ? '興' : market,
			getTypeZhTw(),
			industryZhTw ? industryZhTw + (industryZhTw.endsWith('業') ? '' : '業') : '',
			canShortMargin ? canShortLend ? '可盤下空' : '准融券不可借券盤下空' : canShortLend ? '准借券不可融券盤下空' : '不可盤下空',
			isTerminated ? '下市' : '',
			isSuspended ? '暫停交易' : '',
			abnormal === '正常' || !abnormal ? '' : abnormal + '股',
			isUnusuallyRecommended ? '異常推介股' : ''
		);
	}
	setMeta(index: number, ...metas: string[]): void;
	setMeta(...metas: string[]): void;
	setMeta(startIndex: string | number, ...metas: string[]) {
	...
	quoteTimer: Timer;
	startQuote(timeout = 10_000, slowTimeout = 60_000) {
		if (typeof this.quoteTimer !== 'undefined') { return; }
		const enum QuoteTimerType { Paused, Normal, Slow }
		let quoteTimerType = QuoteTimerType.Normal,
			isClosed = false;
		const quoteHandler = async () => {
			if (document.visibilityState === 'hidden') {
				if (quoteTimerType !== QuoteTimerType.Paused) {
					quoteTimerType = QuoteTimerType.Paused;
			...
			try {
				...
				if (quote.isClosed) {
					if (quoteTimerType !== QuoteTimerType.Slow) {
						quoteTimerType = QuoteTimerType.Slow;
					...
					if (quoteTimerType !== QuoteTimerType.Normal) {
						quoteTimerType = QuoteTimerType.Normal;
						clearInterval(this.quoteTimer);
				...
				const lastUpdatedAtTime = Date.parse(lastUpdatedAt),
					socketLastUpdatedAtTime = Date.parse(this.lastUpdatedAt);
				if (lastUpdatedAtTime > socketLastUpdatedAtTime) {
					...
					if (quote.isClosed) {
						...
						this.setQuote(quote);
						if (lastUpdatedAtTime - socketLastUpdatedAtTime > 2_000) {
							this.socket?.close();
	...
	countLastTradeAt: CountTradeAt;
	getCountTradeColor() {
		return this.countLastTradeAt === CountTradeAt.Bid ? 'palegreen' :
			this.countLastTradeAt === CountTradeAt.Ask ? 'sandybrown' :
				'#ddd';
	}
	get countTradeAt() {
		return this.countLastTradeAt;
	}
	set countTradeAt(countTradeAt: CountTradeAt) {
		...
		if (this.lastTradeAt !== null) {
			const time = Date.now(),
				lastTime = Date.parse(this.lastTradeAt);
			if (time - lastTime > 5_000) {
	...
	countSharedTimer: SharedTimer | null = null;
	startCount() {
		if (this.countSharedTimer !== null) { return; }
		let recentVolumeAtBid = this.tradeVolumeAtBid,
			recentVolumeAtAsk = this.tradeVolumeAtAsk;
		this.countSharedTimer = SharedTimer.start(() => {
			const bidVolume = this.tradeVolumeAtBid - recentVolumeAtBid,
				askVolume = this.tradeVolumeAtAsk - recentVolumeAtAsk;
			if (bidVolume !== askVolume) {
				if (bidVolume > askVolume) {
					this.countTradeAt = CountTradeAt.Bid;
					this.countLastVolume = bidVolume - askVolume;
					this.addDeal(this.tradePrice, this.countLastVolume);
				} else if (askVolume > bidVolume) {
					this.countTradeAt = CountTradeAt.Ask;
				...
				this.countTradeAt = CountTradeAt.Initial;
			}
	...
	async startQuoteSocket() {
		...
		try {
			for await (const { data } of connection.getReadable()) {
				...
				const { data: { info: { lastUpdatedAt, type }, quote } }: QuoteResult = JSON.parse(data);
	...
	async initDealts() {
		...
		if (typeof dealAt === 'undefined') { return; }
		let seperatorTime = Date.parse(dealAt) - 5_000,
			priceSum = 0,
			bidsVolume = 0,
			asksVolume = 0,
			fineVolume = 0;
		const deals: { at: string; price: number; volume: number; countTradeAt: CountTradeAt }[] = [];
				...
				deals.unshift({
					...
					countTradeAt: bidsVolume > asksVolume ?
						...
						asksVolume > bidsVolume ?
							CountTradeAt.Ask :
							CountTradeAt.Initial
				});
				if (deals.length === 14) { break; }
				dealAt = at;
				priceSum = 0;
				bidsVolume = 0;
				asksVolume = 0;
	...
	static getColorHtml(text: string, color: string) {
		return '<span style="color: ' + color + ';">' + text + '</span>';
	}
	static toFixed(n?: number | null) {
...
declare const gridDiv: HTMLDivElement;
addButton.onclick = async () => {
	const fragment = new DocumentFragment,
		group = new QuoteGridGroup(fragment);
