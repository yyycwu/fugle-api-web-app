...
class StrokeGraphicsContainer extends PixiContainer {
    ...
    lineStyle(options, color, alpha, alignment, native) {
        let width;
...
class Alignment {
    static fixedLength(n) {
        return n > 500 ? 0 : n > 50 ? 1 : 2;
    }
    static spaceBetweenHeight(height, ...children) {
        let sum = 0;
    ...
    static rightAlign(width, ...children) {
    ...
    static bottomAlign(height, ...children) {
        for (const child of children) {
            child.y = height - child.height;
        }
    }
    static stackVertical(...children) {
        let y = 0;
        for (const child of children) {
    ...
    static roundChart({ a, o, h, l, c, v, t }, interval = 5 * 60000) {
        ...
        let high = h[0], low = l[0];
        for (let index = 0; index < length;) {
            const t0 = t[index];
            let h0 = h[index], l0 = l[index], vSum = v[index], aSum = a[index] * vSum;
            let i = index + 1;
            if (i < length) {
    ...
    static stackBlock(width, children, updateCallback) {
        const blockHeight = 200, growBlockWidth = 380, maxBlockWidth = 600;
        const colCount = Math.floor(width / growBlockWidth), colWidth = Math.min(maxBlockWidth, width / colCount), endColCount = children.length % colCount, endColWidth = Math.min(maxBlockWidth, width / endColCount), endIndex = children.length - endColCount;
        let x = 0, y = 0;
...
let draggingTarget = null, draggingTargetIndex = 0;
class DraggableContainer extends PixiContainer {
    draggableTarget = this;
    dropped = false;
    constructor(draggableTarget) {
        super();
        if (draggableTarget) {
            this.draggableTarget = draggableTarget;
        }
        let targetLocalPoint = new PIXI.Point, targetGlobalPoint = new PIXI.Point, offsetPoint = new PIXI.Point;
        ...
        this.draggableTarget.on('pointerdown', ({ x, y }) => {
    ...
    drop() {
        const { parent } = this;
        if (parent !== draggingTarget?.parent) {
            return;
        }
...
class ChartBlockContainer extends DraggableContainer {
    ...
    constructor(symbolId) {
        ...
        this.addChild(this.boundingRectGraphics, this.yAxisContainer.addChild(this.highPriceText, this.avgPriceText, this.lowPriceText), this.chartContainer.addChild(this.gridGraphics, this.kChartGraphicsContainer, this.avgPriceGraphicsContainer, this.kChartVolGraphics, this.volSumGraphics, this.kInspectLineGraphics, this.metaContainer.addChild(this.highLimitText, this.referPriceText, this.lowLimitText, this.amplitudeText), this.kDetailBoxContainer.addChild(this.kDetailBoxBgGraphics, this.kHighText, this.kUpText, this.kDownText, this.kLowText, this.kVolText), this.symbolIdContainer.addChild(this.symbolIdText, this.symbolIdTextText)));
        ...
        this.addEventListener('click', ({ detail }) => {
            if (detail === 2) {
                this.destroy();
            }
        });
    ...
    async update() {
        if (this.scheduledAnimationFrame === false &&
        ...
        const { data: { info: { lastUpdatedAt }, chart } } = await FugleApi.chart(this.symbolId, { signal }), referPrice = parseFloat(this.referPriceText.text), { high = referPrice, low = referPrice, chart: { a, o, h, l, c, v, t } } = Alignment.roundChart(chart, 5 * 60000), fixedLength = Alignment.fixedLength(low), fixedHigh = high.toFixed(fixedLength), fixedLow = low.toFixed(fixedLength);
        ...
        const stickWidth = chartWidth / t.length, thickWidth = Math.max(2, stickWidth - 2), thinWidth = Math.max(2, thickWidth / 5), stickXOffset = thickWidth / 2, stickYScale = chartHeight / (high - low), maxVol = Math.max(...v), volYScale = maxVol > halfChartHeight ? halfChartHeight / maxVol : 1, halfVolStickWidth = Math.max(1, thickWidth / 4);
        ...
        for (let i = 0; i < t.length; i++) {
            ...
            const color = c[i] > o[i] ? 0xfe8888 : c[i] < o[i] ? 0x88fe88 : 0xfefefe, x = i * stickWidth + stickXOffset;
            ...
            let y1 = chartHeight - (c[i] - low) * stickYScale, y2 = chartHeight - (o[i] - low) * stickYScale;
            if (Math.abs(y1 - y2) < 1) {
            ...
            kChartGraphics.on('pointerenter', () => {
                ...
                if (this.kInspectLineGraphics.geometry.graphicsData.length === 1) {
                    ...
                    const geometry = this.kInspectLineGraphics.geometry;
                    geometry.invalidate();
                }
            ...
            const volumeDifference = i > 0 ? v[i] - v[i - 1] : v[i + 1] - v[i], volumeDifferenceColor = volumeDifference > 0 ? 0xff5050 : volumeDifference < 0 ? 0x00ff00 : 0xdddddd;
...
function updateChartBlockContainerBounds(chartBlockContainer) {
...
function resizeChartApp() {
    const children = chartApp.stage.children, { offsetHeight } = mainDiv;
